...
auditlog='/opt/zimbra/log/audit.log' # Caso deseje filtrar por um mês, concatenar ao final: AAAA-MM-
zimbralog='/var/log/zimbra' # Caso deseje filtrar por um mês, concatenar ao final: -AAAAMM
report='./report.txt'
...
# Copies the logs to local and change the log path
cp ${auditlog}* ${export_dir}/log-files/audit-log
cp ${zimbralog}* ${export_dir}/log-files/zimbra-log
auditlog="${export_dir}/log-files/audit-log/"
zimbralog="${export_dir}/log-files/zimbra-log/"
...
for email in $(cat ${email_accounts}); do
  ...
  echo "Status: ${acc_status}" | tee -a ${report}
  ...
  echo "Timestamp do último login no webmail: ${dt_last_login}" | tee -a ${report}
  ...
  echo "Timestamp da última alteração de senha: ${dt_pw_change}" | tee -a ${report}
  ...
  echo "Configuração de encaminhamento: ${forwarding}" | tee -a ${report}
  ...
  echo "Endereços IP distintos que realizaram login com sucesso: (Total, IP):" | tee -a ${report}
  ...
  cat ${export_dir}/${account}/${account}_audit-login-successfully-IPs-temp.txt | sort | uniq -c | sort -nr | tee -a ${report}
